...
def analyze_with_stockfish(fen, stockfish_path=r"stockfish/stockfish.exe"):
    try:
        # Inizializza Stockfish
        engine = chess.engine.SimpleEngine.popen_uci(stockfish_path)
        
        # Crea una board a partire dal FEN
        board = chess.Board(fen)
        
        # Esegui l'analisi
        result = engine.analyse(board, chess.engine.Limit(time=5.0))
        
        # Stampa i risultati
        print("\nAnalisi Stockfish:")
        print(f"Mossa migliore: {result.get('pv', [None])[0]}")
        print(f"Punteggio: {result.get('score', 'N/A')} (centipawn)")
        print(f"Profondità: {result.get('depth', 0)} livelli")
